"""
Mediation Model 1: Total effect.
Y = EMP_Mean or EMP_1 ~ condition_avatar + controls (no mediator).
Output: model1_total_effect/EMP_Mean/, model1_total_effect/EMP_1/
"""
import os
import pandas as pd
...
def main() -> None:
    ...
    required_columns = [
        "EMP_Mean", "EMP_1", "condition_avatar",
    ...
    print(f"Model 1 (Total effect). Rows: {len(df_model)}")

    output_dir = os.path.join(base_dir, "model1_total_effect")
    formula_base = "{} ~ condition_avatar + extraversion_score + agreeableness_score + C(Sex) + C(Q('Ethnicity simplified')) + total_tokens"
