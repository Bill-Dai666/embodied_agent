"""
Mediation Model 2: Path a (mediator equation).
A = anthropomorphism_score ~ condition_avatar + controls.
Output: model2_path_a/anthropomorphism/
"""
import os
import pandas as pd
...
def build_and_save(df: pd.DataFrame, outcome: str, formula: str, output_dir: str, outcome_subdir: str | None = None) -> None:
    """outcome = column name for DV; outcome_subdir = folder name (default outcome)."""
    model = smf.ols(formula=formula, data=df).fit()
    folder = outcome_subdir if outcome_subdir is not None else outcome
    outcome_dir = os.path.join(output_dir, folder)
    ...
    print(f"  {folder}: coeffs, fit_stats, predictions -> {outcome_dir}")
...
def main() -> None:
    ...
    required_columns = [
        "anthropomorphism_score", "condition_avatar",
    ...
    print(f"Model 2 (Path a: T -> A). Rows: {len(df_model)}")

    output_dir = os.path.join(base_dir, "model2_path_a")
    formula = "anthropomorphism_score ~ condition_avatar + extraversion_score + agreeableness_score + C(Sex) + C(Q('Ethnicity simplified')) + total_tokens"
    build_and_save(df_model, "anthropomorphism_score", formula, output_dir, outcome_subdir="anthropomorphism")
